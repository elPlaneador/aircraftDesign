...
# Propeller
prop_type_txt = '20 x 18, 8000 rpm'
prop_type_txt = prop_type_txt.lower().replace('x', ' ').replace(',', ' ').replace('rpm', '')
dia_inch      = float(prop_type_txt.split()[0])
way_inch      = float(prop_type_txt.split()[1])
rpm           = float(prop_type_txt.split()[2])
...
def prop_way(way_inch):
    way_in_meter = inch_to_meter(way_inch) 
    return way_in_meter  # meter per second for eta_prop = 100 %

def prop_preferences(dia_inch, way_inch, rpm):
    my_prop_way   = prop_way(way_inch)
    my_prop_speed = my_prop_way * rpm / 60 # Way per second
    my_prop_d     = inch_to_meter(dia_inch)
    my_prop_circ_speed = my_prop_d * pi * rpm / 60
    my_prop_tip_mach   = sqrt(my_prop_circ_speed**2 + my_prop_speed**2) / 343.0 # tip speed in mach

    return my_prop_way, my_prop_speed, my_prop_tip_mach



prop_way, prop_speed, prop_tip_mach = prop_preferences(dia_inch, way_inch, rpm)
...
print()
print('Prop Way theoretically:') 
print('Speed:      {:>5s} m/s'.format(str(round(prop_speed, 1))))
print('Blade_t Ma: {:>5s} '.format(str(round(prop_tip_mach, 1))))
print('RPM:        {:>5s} rpm'.format(str(round(rpm, 1))))
